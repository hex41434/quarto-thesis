...
![Shell mesj - 2D image](/img/Chp1/karo_2D.png){#fig-karo-2d fig-align="center" width="80%"}
...
![deformation modeling of the shell mesh](/img/Chp1/karo_3d.png){#fig-shell-sdf fig-align="center" width="60%"}

## Sequential Deformation

Contrary to direct deformation, sequential deformation takes place over multiple stages, where a structure or material progressively changes under successive loads or forces @fig-seq-def. Each stage adds new stresses or strains, resulting in cumulative impacts on the material’s shape or integrity. In production, sequential deformation is typically seen in processes such as incremental forming, deep drawing, or multi-stage forging, where materials are gradually shaped to form complex geometries while ensuring proper strain distribution and reducing defects. To model this type of process, it is necessary to first gather data to train the network on a series of deformation steps. The modeling approach must be adapted to account for these multiple steps, allowing the network to predict deformations over time.

The previous method, designed for cases where different forces act on the initial object to generate deformations, is unsuitable for modeling multi-stage deformations. In multi-stage deformation, the sequence and interaction of applied forces are interdependent. To address this challenge, a deep reinforcement learning (DeepRL) approach was introduced. In this method, the problem is viewed as a series of states and actions, where the optimal action sequence is determined by assessing the rewards associated with each state. The object’s geometry represents the state, while the applied force is the action.

To solve this using deep reinforcement learning, a large number of actions must be sampled and applied to various states, with rewards assigned based on how closely the resulting state matches the target. Higher rewards indicate that the current state is closer to the target, signifying a better choice of actions. Over time, methods are applied to repeat these steps and find the optimal action sequence. Due to the extensive computational demands of handling numerous state-action pairs, some approaches employ compressed state representations to reduce the computational load and improve efficiency.

![Sequential Deformation in 5 Steps](/img/Chp3/MSD.png){#fig-seq-def fig-align="center" width="60%"}

To achieve this, several approaches for compressing the states were explored:

1.  Treating the deformed mesh surface as a 2D depth image : Image-Based MSD

2.  Compressing the mesh using a mesh autoencoder (CoMA) : Mesh-Based MSD 

3.  neural-encoded object representation to model multi-step deformations : Implicit MSD
